...
class Audio2landmark_model():
    ...
    def __init__(self, options, jpg_shape=None):
        '''
        Init model with options
        '''
    ...
    def __calib_baseline_pred_fls_old_(self, baseline_pred_fls, residual_face_id, aus):
        ...
        baseline_pred_fls[:, 48 * 3::3] *= self.options.amp_lip_x  # mouth x
        baseline_pred_fls[:, 48 * 3 +
                          1::3] *= self.options.amp_lip_y  # mouth y
    ...
    def __train_pass__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            keys = self.options.reuse_train_emb_list
            if(len(keys) == 0):
                keys = ['audio_embed']
            for key in keys:
                # load saved emb
                if au_emb is None:
                    raise ValueError('test_emb')
                ...
                files.append(fake_fls_np)

                # ''' Visualize result in landmarks '''
                if(vis_fls):
                    print('vis_fls', os.path.join('examples',
                                                  video_name.split('\\')[-1].split('/')[-1]+'.wav'))
    ...
    def test(self, au_emb=None):
        with torch.no_grad():
            files = self.__train_pass__(au_emb, vis_fls=False)  # True
        return files
